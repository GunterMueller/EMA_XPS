/****************************************************
*   constraint.c  Hans Groschwitz        19.07.95   *
*                 Karsten Vossberg                  *
*                 Klaus Senf                        *
*                                                   *
...
/*** Globals for Constraint-Editor ***************************************/

static Widget c_shellW ;
static Widget c_nameW, c_artW;      
static Widget c_interfaceW, c_bodyW, c_conditionW, c_namelabelW;
static Widget c_labelbodyW, c_formconditionW, c_forminterfaceW;
...
c_Change_True( void ) {
   ...
   if( c_selected_kb )
      C_set_modified( c_selected_kb, MOD_CONSTRAINT, True );
}
...
c_Change_False( void ) {
   ...
   if( c_selected_kb )
      C_set_modified( c_selected_kb, MOD_CONSTRAINT, False );
}
...
c_edit( Boolean ability ) {
   ...
   XtSetSensitive( c_conditionW, ability );
   c_editable = ability;
}
...
c_NoSelect( void ) {
   ...
   XmTextSetString( c_conditionW, "" );
   XmTextShowPosition( c_conditionW, 0 );
...
/******************************************************************************
*  LISP->C:
*  Reads the definition of a primitive constraint 
*  --> dispatch.c
******************************************************************************/

void 
emaxps_constraint_def( void ) {
   ...
   if( err ) {
      fprintf( stderr, "constraint_def: too few args\n" );
      LISP_sndReply( ":ERROR" );
      XtFree( p );
      return;
   }
   if( GetNextPartStr( v[i-1] ) ) {
      fprintf( stderr, "constraint_def: too many args\n" );
      LISP_sndReply( ":ERROR" );
      XtFree( p );
      return;
   }
   ...
   if (strlen(c_condition)>1)
     XmTextSetString( c_conditionW, c_condition  );
   else
     XmTextSetString( c_conditionW,  "T" );
   if (c_selected_co)
      XtFree( c_selected_co );
   c_selected_co=XtMalloc(1+strlen(c_name));
   strcpy(c_selected_co,c_name);                         
   c_Change_False();
   c_edit(True);
   XtSetSensitive( c_LoeschenW, True);
...
/*****************************************************************************
*   Callback zur Anzeige der Definition einer primitiven Constraint
*   im Constrainteditor 
*******************************************************************************/

void 
constraintEditorAnzeigenCB( void ) {
   char *buf;

   buf = XtMalloc( 256 + strlen(c_selected_kb) + strlen(c_selected_kb) );
   sprintf( buf, "($send (symbol-value '%s) \n\
      :hole-primitive-constraint-definition-mit-menue \n\
      '%s)",
...
void 
constraintEditorQuitCB ( void ) {
                               
   CONSTRAINT_hide();
} 

/*****************************************************************************
*   Callback fuer den Menuegruppe "Primitive-Constraint loeschen .."
*   im Constrainteditor 
******************************************************************************/

void 
constraintEditorPrimLoeschenCB ( void ) {
...
c_LoadWarningokCB( void ) {
   ...
   char *interface;
   char *body;             
   char *condition;  /* only for primitive constraints */
  
   interface = XmTextGetString( c_interfaceW );    
   body      = XmTextGetString( c_bodyW );
   condition = XmTextGetString( c_conditionW );
   if (strlen(condition)<1){
       condition=(char*) XtMalloc(2);
       condition="T";
   }    
   
   buf=XtMalloc(256+strlen(interface)
        +strlen(body)+strlen(condition));
   sprintf(buf, "($send (symbol-value '%s) \n\
      ...
      '(:relation %s) \n\
      '(:condition %s))",
      c_selected_kb, c_selected_kb, c_selected_co, interface, body, condition ); 
   XtUnmanageChild(c_LoadWarningW  );
   c_Change_False();
   LISP_sndCmd( buf );
...
c_DelWarningokCB( void ) {
   ...
   sprintf( buf, "($send (symbol-value '%s) \n\
      :loesche-primitive-constraint '%s '%s)",
...
******************************************************************************
**                                                                          **
**                Erzeugen des Constrainteditors                            ** 
**                                                                          **
******************************************************************************
******************************************************************************/

void 
CONSTRAINT_create( Widget parent ) {
   Widget formW,form1W;
    ...
    */
   c_shellW = XtAppCreateShell( "Constraint-Editor", "HereClassIsIgnored",
      topLevelShellWidgetClass, XtDisplay( parent ), args, n );

   /*** install callback for the CLOSE-Button of the mwm-border menu ***/
   XmAddWMProtocolCallback( c_shellW, WM_DELETE_WINDOW,
      (XtPointer)CONSTRAINT_hide, NULL );        /* Pop down ... */

   /******************* Hauptfenster **********************************/
   n = 0;
   formW = XmCreateForm (c_shellW, "Constrainteditor", args, n);
   XtManageChild (formW);            
   ...
   XtAddCallback( formW, XmNhelpCallback,
      (XtPointer)HELP_show, (XtPointer)HELP_CONSTRAINT_MENU );
  ...
  /******** Constraint-SelectionMenu-MessageBox-Widget ******************/
   ...
   *******************************************************************/
   ...
   XtSetArg( args[n], XmNtopAttachment,               XmATTACH_FORM ); n++;  
   XtSetArg( args[n], XmNalignment,            XmALIGNMENT_BEGINNING ); n++;
   c_labelbodyW = XmCreateLabel( formEd2W, "Relation", args, n );
   ...
   /********* 3. Editorfenster fuer die Aktivierungsbedingung*************/
   /*** Form-Widget ***/
   n = 0;
   XtSetArg( args[n], XmNpaneMaximum,                         140 ); n++;
   c_formconditionW = XmCreateForm( panedwindow, "formEditorWin", args, n );

   XtManageChild( c_formconditionW );
   ...
   XtSetArg( args[n], XmNtopAttachment,                XmATTACH_FORM ); n++;  
   XtSetArg( args[n], XmNalignment,            XmALIGNMENT_BEGINNING ); n++;
   labelEd3W = XmCreateLabel( c_formconditionW, 
                              "Aktivierungsbedingung", args, n );
   XtManageChild( labelEd3W );

   n = 0; 
   XtSetArg( args[n], XmNleftAttachment,              XmATTACH_FORM ); n++;
   XtSetArg( args[n], XmNrightAttachment,             XmATTACH_FORM ); n++;
   XtSetArg( args[n], XmNtopAttachment,             XmATTACH_WIDGET ); n++;
   XtSetArg( args[n], XmNtopWidget,                       labelEd3W ); n++;  
   XtSetArg( args[n], XmNbottomAttachment,            XmATTACH_FORM ); n++;
   XtSetArg( args[n], XmNscrollBarDisplayPolicy,    XmAS_NEEDED ); n++;
   XtSetArg( args[n], XmNscrollingPolicy,           XmAUTOMATIC ); n++;
   XtSetArg( args[n], XmNwordWrap,                         True ); n++;
   XtSetArg( args[n], XmNeditMode,            XmMULTI_LINE_EDIT ); n++;
   XtSetArg( args[n], XmNautoShowCursorPosition,           True ); n++;
   c_conditionW = XmCreateScrolledText( c_formconditionW, "EditorWin", args, n );
   XtAddCallback( c_conditionW, XmNhelpCallback,
      (XtPointer)HELP_showBrowserCB, NULL );
   XtAddCallback( c_conditionW, XmNvalueChangedCallback,
     (XtPointer)c_changedCB, NULL );
   XtManageChild( c_conditionW );
...
/****************************************************
*   Change Sensitivity of the Constraint Editor     *
*   (accessable from outside...)                    *
****************************************************/

void
CONSTRAINT_ready( void ) {
...
/****************************************************
*   Shows and updates the Constraint Editor         *
****************************************************/

void
CONSTRAINT_show( void ) {
