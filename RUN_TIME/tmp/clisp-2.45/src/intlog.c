...
/* (LOGIOR x y), with x, y being integers.
 result: integer.
 can trigger GC */
local maygc object I_I_logior_I (object x, object y) { GCTRIGGER2(x,y); {
...
/* (LOGXOR x y), with x, y being integers.
 result: Integer.
 can trigger GC */
local maygc object I_I_logxor_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
...
/* (LOGAND x y), with x, y being integers.
 result: integer.
 can trigger GC */
local maygc object I_I_logand_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
      (as_oint(x) & as_oint(y));
...
/* (LOGEQV x y), with x, y being integers.
 result: integer.
 can trigger GC */
local maygc object I_I_logeqv_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
      ( ~(as_oint(x) ^ as_oint(y))
...
/* (LOGNAND x y), with x, y being integers.
 result: integer.
 can trigger GC */
local maygc object I_I_lognand_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
...
/* (LOGNOR x y), with x, y being integers.
 result: Integer.
 can trigger GC */
local maygc object I_I_lognor_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
      ((as_oint(x) | as_oint(y)) ^ FN_value_vz_mask);
  } else {
...
/* (LOGANDC2 x y), with x, y being integers.
 result: Integer.
 can trigger GC */
local maygc object I_I_logandc2_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
      ((as_oint(x) & ~as_oint(y)) | ((oint)fixnum_type << oint_type_shift));
  } else if (posfixnump(x)) {
...
#else
local maygc object I_I_logandc1_I (object x, object y) { GCTRIGGER2(x,y); {
...
/* (LOGORC2 x y), with x, y being integers.
 result: Integer.
 can trigger GC */
local maygc object I_I_logorc2_I (object x, object y) { GCTRIGGER2(x,y); {
  if (I_fixnump(x) && I_fixnump(y)) { /* both fixnums -> very simple: */
    return as_object /* return bitwise as fixnum */
      ((as_oint(x) | ~as_oint(y))
       & (((oint)fixnum_type << oint_type_shift) | FN_value_vz_mask));
...
#else
local maygc object I_I_logorc1_I (object x, object y) { GCTRIGGER2(x,y); {
...
/* (LOGNOT x), with x being an integer.
 result: Integer.
 can trigger GC */
local maygc object I_lognot_I (object x) { GCTRIGGER1(x); {
...
/* (BOOLE op x y), with x and y being integers and op an object.
 ...
 can trigger GC */
local maygc object OP_I_I_boole_I (object op, object x, object y) { GCTRIGGER3(op,x,y); {
...
/* (INTEGER-LENGTH x), with x being an integer. result: Integer >=0.
 ...
 can trigger GC */
local maygc object I_integer_length_I (object x) { GCTRIGGER1(x); {
  if (I_fixnump(x)) {
    {var uintL bitcount = 0;
    {var uintV x_ = FN_to_V(x); /* x as intVsize-bit-number */
    if (FN_V_minusp(x,(sintV)x_))
