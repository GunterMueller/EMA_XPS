...
/* Starts a secondary driver (Read-Eval-Print-Loop)
 ...
 can trigger GC */
global maygc void break_driver (bool continuable_p) { GCTRIGGER(); {
...
/* climb n times or as far as possible, according to table
 ...
 can trigger GC*/
local maygc Values climb_stack (const climb_fun_t* table) { GCTRIGGER(); {
...
local maygc void print_back_trace (const gcv_object_t* stream_,
                                   const struct backtrace_t *bt, uintL index) { GCTRIGGER(); {
...
{ GCTRIGGER(); { /* run along the stack upwards */
