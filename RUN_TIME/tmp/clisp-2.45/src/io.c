...
local inline maygc void syntax_table_put (object table, chart c, uintB value) { GCTRIGGER1(table); {
  if (as_cint(c) < small_char_code_limit)
...
/* UP: returns the standard (original) readtable.
 ...
 can trigger GC */
local maygc object orig_readtable (void) { GCTRIGGER(); {
...
local maygc object copy_readtable_contents (object from_readtable,
                                            object to_readtable) { GCTRIGGER2(from_readtable,to_readtable); {
  /* copy the case-slot: */
...
/* UP: copies a readtable
 ...
 can trigger GC */
local maygc object copy_readtable (object from_readtable) { GCTRIGGER1(from_readtable); {
...
/* UP: Initializes the reader.
 init_reader();
 can trigger GC */
global maygc void init_reader (void) { GCTRIGGER(); {
...
/* error, if argument is no Readtable.
 ...
 can trigger GC */
local maygc object check_readtable (object obj) { GCTRIGGER1(obj); {
...
/* UP: checks an optional readtable-argument,
 ...
 < result: readtable
 can trigger GC */
local maygc object test_readtable_arg (object readtable) { GCTRIGGER1(readtable); {
  if (!boundp(readtable)) {
...
/* UP: checks an optional readtable-argument,
 ...
 < result: readtable
 can trigger GC */
local maygc object test_readtable_null_arg (object readtable) { GCTRIGGER1(readtable); {
  if (!boundp(readtable)) {
    get_readtable(readtable=); /* the current readtable is default */
...
/* UP: checks the arguments disp-char and sub-char.
 ...
 can trigger GC */
local maygc object test_disp_sub_char (gcv_object_t* argsp) { GCTRIGGER(); {
...
/* UP: read up to the next non-whitespace-character, without consuming it.
 ...
 can trigger GC */
local maygc object wpeek_char_eof (const gcv_object_t* stream_) { GCTRIGGER(); {
  while (1) {
...
/* UP: delivers two buffers.
 ...
 can trigger GC */
local maygc void get_buffers (void) { GCTRIGGER(); {
  /* Mechanism:
...
local maygc void read_token (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: treatment of read-macro-character:
 ...
 can trigger GC */
local maygc Values read_macro (object ch, const gcv_object_t* stream_) { GCTRIGGER1(ch); {
  {var object readtable;
...
/* UP: reads an object.
 ...
 can trigger GC */
local maygc object read_internal (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: reads an Object, with SYS::*READ-RECURSIVE-P* /= NIL
 ...
 can trigger GC */
local maygc object read_recursive (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: reads an Object, with SYS::*READ-RECURSIVE-P* /= NIL
 ...
 < result: read Object
 can trigger GC */
local maygc object read_recursive_no_dot (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: Reads an Object, with SYS::*READ-RECURSIVE-P* = NIL .
 ...
 can trigger GC */
local maygc object read_top (const gcv_object_t* stream_, object whitespace_p) { GCTRIGGER1(whitespace_p); {
#if STACKCHECKR
...
global maygc object stream_read (const gcv_object_t* stream_,
                                 object recursive_p, object whitespace_p) { GCTRIGGER2(recursive_p,whitespace_p); {
...
local maygc object read_delimited_list(const gcv_object_t* stream_, object endch,
                                       object ifdotted) { GCTRIGGER2(endch,ifdotted); {
 #if STACKCHECKR
  {var gcv_object_t* STACKbefore = STACK; /* retain STACK for later */
 #endif
...
/* stream_ = check_stream_arg(stream_);
 ...
 < stream_: &stream
 can trigger GC */
static inline maygc gcv_object_t* check_stream_arg (gcv_object_t *stream_) { GCTRIGGER(); {
...
/* reads an Object and creates a list with two elements.
 ...
 can trigger GC */
local maygc Values list2_reader (const gcv_object_t* stream_) { GCTRIGGER(); {
  dynamic_bind(S(read_recursive_p),T);
...
/* UP: checks the absence of Infix-Argument n
 ...
 modifies STACK
 can trigger GC */
local maygc gcv_object_t* test_no_infix (void) { GCTRIGGER(); {
...
/* (defun radix-1 (stream sub-char n base)
   ...
   can trigger GC */
local maygc Values radix_2 (uintWL base) { GCTRIGGER(); {
...
/* UP: for #B #O #X
 ...
 < mv_space/mv_count: values
 can trigger GC */
local maygc Values radix_1 (uintWL base) { GCTRIGGER(); {
  {var gcv_object_t* stream_ = check_stream_arg(&STACK_2);
...
/* (set-dispatch-macro-character #\##\=
 ...
 can trigger GC */
local maygc object lookup_label (void) { GCTRIGGER(); {
...
/* UP: for #+ und #-
 ...
 < mv_space/mv_count: values
 can trigger GC */
local maygc Values feature (uintWL demandvalue) { GCTRIGGER(); {
  {var gcv_object_t* stream_ = test_no_infix(); /* n must be NIL */
...
/* UP: checks an Input-Stream-Argument.
 ...
 can trigger GC */
local maygc void check_istream (gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: for READ and READ-PRESERVING-WHITESPACE
 ...
 < mv_space/mv_count: values
 can trigger GC */
local maygc Values read_w (object whitespace_p) { GCTRIGGER1(whitespace_p); {
...
/* ---------------------- common sub-routines ----------------------------
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_uint (const gcv_object_t* stream_, uintL x) { GCTRIGGER(); {
...
/* UP: outputs a Nibble hexadecimally (with 1 hex-digit) to stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_hex1 (const gcv_object_t* stream_, uint4 x) { GCTRIGGER(); {
...
/* UP: outputs a byte hexadecimally (with 2 hex-digits) to stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_hex2 (const gcv_object_t* stream_, uint8 x) { GCTRIGGER(); {
...
/* UP: outputs an address with 24 Bit hexadecimally (with 6 hex-digits)
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_hex6 (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* UP: outputs an address with 32 bit hexadecimally (with 8 hex-digits)
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_hex8 (const gcv_object_t* stream_, uintP x) { GCTRIGGER(); {
...
local maygc void write_sstring_ab (const gcv_object_t* stream_, object string,
                                   uintL start, uintL len) { GCTRIGGER1(string); {
  if (len==0) return;
  pushSTACK(string);
...
/* UP: prints simple-string elementwise to stream.
 ...
 < stream: Stream
 can trigger GC */
global maygc void write_sstring (const gcv_object_t* stream_, object string) { GCTRIGGER1(string); {
...
/* UP: prints string elementwise to stream.
 ...
 < stream: Stream
 can trigger GC */
global maygc void write_string (const gcv_object_t* stream_, object string) { GCTRIGGER1(string); {
...
local maygc void write_sstring_invert(const gcv_object_t* stream_,
                                      object string) { GCTRIGGER1(string); {
  sstring_un_realloc(string);
...
local maygc void write_sstring_case_ext (const gcv_object_t* stream_,
                                         ...
                                         bool case_inverted) { GCTRIGGER1(string); {
  if (case_sensitive) {
    if (case_inverted)
...
/* UP: prints a number of Spaces to stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void spaces (const gcv_object_t* stream_, object scount) { GCTRIGGER1(scount); {
  {var uintV count;
 #ifdef IO_DEBUG
...
local maygc void pphelp_newline (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: prints parenthesis '(' to the stream and possibly memorizes
 ...
 changes STACK
 can trigger GC */
local maygc void paren_open (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
...
/* UP: Prints parenthesis ')' to the Stream, possibly at the memorized
 ...
 changes STACK
 can trigger GC */
local maygc void paren_close (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (!PPHELP_STREAM_P(stream)) { /* normal Stream */
    write_ascii_char(stream_,')');
  } else {                  /* Pretty-Print-Help-Stream */
...
/* UP: empties a Pretty-Print-Help-Stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void justify_empty_1 (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: empties the content of Pretty-Print-Hilfsstream into the Variable
 ...
 < stream: Stream
 can trigger GC */
local maygc void justify_empty_2 (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  {var object new_cons;
...
/* UP: prints space, which can be stretched with Justify.
 ...
 < stream: Stream
 can trigger GC */
local maygc void justify_space (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: Finalizes a Justify-Block, determines the shape of the Block and
 ...
 < stream: Stream
 can trigger GC */
local maygc void justify_end_fill (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: finalizes a justify-block, determines the shape of the block and
 ...
 < stream: stream
 can trigger GC */
local maygc void justify_end_linear (const gcv_object_t* stream_) { GCTRIGGER(); {
  if (!PPHELP_STREAM_P(*stream_)) { /* normal stream -> nothing to do */
  } else {                          /* Pretty-Print-Help-Stream */
    justify_empty_2(stream_);       /* save stream content */
...
/* UP: abbreviate the remainder with "..."
 ...
 < stream: stream
 can trigger GC */
local maygc void triple_dots (const gcv_object_t* stream_) { GCTRIGGER(); {
  JUSTIFY_LAST(true);
  write_ascii_char(stream_,'.');
  write_ascii_char(stream_,'.');
  write_ascii_char(stream_,'.');
...
/* UP: abbreviate the remainder with ".."
 ...
 < stream: stream
 can trigger GC */
local maygc void double_dots (const gcv_object_t* stream_) { GCTRIGGER(); {
  JUSTIFY_LAST(true);
...
/* UP: verifies, if an object is circular, and prints it in
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_circle (const gcv_object_t* stream_, object obj, pr_routine_t* pr_xxx) { GCTRIGGER1(obj); {
...
/* UP: return the number of spaces available on the current line in this stream
 NIL means unlimited
 can trigger GC */
local maygc object space_available (object stream) { GCTRIGGER1(stream); {
...
local maygc inline bool string_fit_line_p (const gcv_object_t *stream_,
                                           object list, uintL offset) { GCTRIGGER1(list); {
...
local maygc void pr_enter_1 (const gcv_object_t* stream_, object obj,
                             pr_routine_t* pr_xxx) { GCTRIGGER1(obj); {
...
local maygc void prin_object_ki (const gcv_object_t* stream_, object obj,
                                 pr_routine_t *printer) { GCTRIGGER1(obj); {
 restart_it:
...
{ GCTRIGGER1(obj); { prin_object_ki(stream_,obj,&prin_object_dispatch); }}
...
{ GCTRIGGER1(obj); { if (!nullpSv(print_pretty))
...
/* UP: print a symbol into a stream
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_symbol (const gcv_object_t* stream_, object sym) { GCTRIGGER1(sym); {
  /* query *PRINT-ESCAPE*: */
...
local inline maygc bool pr_fasl_special (const gcv_object_t* stream_, chart c,
                                         bool *pending_newline) { GCTRIGGER(); {
...
local maygc void pr_symbol_part (const gcv_object_t* stream_, object string,
                                 bool case_sensitive, bool case_inverted) { GCTRIGGER1(string); {
...
/* UP: prints Simple-String like a part of a Symbol.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_like_symbol (const gcv_object_t* stream_, object string) { GCTRIGGER1(string); {
  /* query *PRINT-ESCAPE*: */
  if (!nullpSv(print_escape) || !nullpSv(print_readably)) {
...
/* -------- Characters --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_character (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  /* query *PRINT-ESCAPE*: */
  if (!nullpSv(print_escape) || !nullpSv(print_readably)) {
...
/* UP: prints string to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_string (const gcv_object_t* stream_, object string) { GCTRIGGER1(string); {
  {var uintL len = vector_length(string); /* length */
  {var uintL offset = 0;        /* Offset of string in the data-vector */
...
/* UP: prints list to stream, NIL as ().
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_list (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
...
/* UP: print a Cons to a stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_cons (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
  {                             /* treat special case: */
...
local maygc void pr_list_quote (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
  /* list = (QUOTE object) */
...
local maygc void pr_list_function (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
...
local maygc void pr_list_backquote (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
...
local maygc void pr_list_bothsplice (const gcv_object_t* stream_,
                                     object list, object ch) { GCTRIGGER2(list,ch); {
...
local maygc void pr_list_splice (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
  /* list = (SPLICE object) */
  pr_list_bothsplice(stream_,list,ascii_char('@'));
}}

local maygc void pr_list_nsplice (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
  /* list = (NSPLICE object) */
  pr_list_bothsplice(stream_,list,ascii_char('.'));
}}

local maygc void pr_list_unquote (const gcv_object_t* stream_, object list) { GCTRIGGER1(list); {
  /* list = (UNQUOTE object) */
  pushSTACK(Car(Cdr(list)));     /* save (second list) */
  write_ascii_char(stream_,','); /* print ',' */
  list = popSTACK();
...
/* UP: Print a pair in cons-like (car . cdr) notation.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_pair (const gcv_object_t* stream_, object car, object cdr) { GCTRIGGER2(car,cdr); {
  LEVEL_CHECK;
  {
    {var uintL length_limit = get_print_length(); /* *PRINT-LENGTH*-limit */
...
/* -------- Numbers --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_real_number (const gcv_object_t* stream_, object number) { GCTRIGGER1(number); {
...
/* UP: prints number to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_number (const gcv_object_t* stream_, object number) { GCTRIGGER1(number); {
...
local maygc void pr_unreadably (const gcv_object_t* stream_, object obj,
                                gcv_object_t *string_, pr_routine_t printer) { GCTRIGGER1(obj); {
  LEVEL_CHECK;
  pushSTACK(obj);                    /* save object */
...
local maygc void pr_unreadably_2 (const gcv_object_t* stream_, object obj1,
                                  object obj2, gcv_object_t *string_) { GCTRIGGER2(obj1,obj2); {
  LEVEL_CHECK;
  pushSTACK(obj1);              /* save */
  pushSTACK(obj2);              /* save */
...
/* -------- Arrays when *PRINT-ARRAY*=NIL --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_array_nil (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
local maygc void pr_sbvector_ab (const gcv_object_t* stream_, object bv,
                                 uintL start, uintL len) { GCTRIGGER1(bv); {
  {var uintL index = start;
...
/* UP: prints bit-vector to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_bvector (const gcv_object_t* stream_, object bv) { GCTRIGGER1(bv); {
  /* query *PRINT-ARRAY*: */
...
/* -------- Generic Vectors --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_vector (const gcv_object_t* stream_, object v) { GCTRIGGER1(v); {
  /* query *PRINT-ARRAY*: */
  if (!nullpSv(print_array) || !nullpSv(print_readably)) {
    /* print v elementwise: */
    LEVEL_CHECK;
...
local maygc void pr_kvtable (const gcv_object_t* stream_, gcv_object_t* kvt_,
                             uintL index, uintL count) { GCTRIGGER(); {
  {var uintL length = 0;
  {var uintL length_limit = get_print_length(); /* *PRINT-LENGTH*-limit */
  while (1) {
...
/* -------- Nil-Vectors --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_nilvector (const gcv_object_t* stream_, object v) { GCTRIGGER1(v); {
  {var uintL len = vector_length(v); /* vector-length */
  write_ascii_char(stream_,'#'); write_ascii_char(stream_,'A');
  PAREN_OPEN;                 /* print '(' */
  INDENT_START(3);         /* indent by 3 characters because of '#A(' */
...
local maygc void pr_array_elt_bvector (const gcv_object_t* stream_, object obj,
                                       pr_array_info_t* info)
{ GCTRIGGER1(obj); { /* simple-bit-vector */
...
/* UP: prints multi-dimensional array to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_array (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  /* query *PRINT-ARRAY* : */
  if (!nullpSv(print_array) || !nullpSv(print_readably)) {
...
local maygc void pr_sharp_dot (const gcv_object_t* stream_,object obj) { GCTRIGGER1(obj); {
...
/* UP: prints CLOS-instance to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_instance (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  pushSTACK(obj);              /* save obj */
...
local maygc void pr_structure_external (const gcv_object_t* stream_,
                                        object structure, object function) { GCTRIGGER2(structure,function); {
  LEVEL_CHECK;
  {var object stream = *stream_;
...
/* UP: prints structure to stream.
 ...
 can trigger GC */
local maygc void pr_structure (const gcv_object_t* stream_, object structure) { GCTRIGGER1(structure); {
...
local maygc void pr_structure_default (const gcv_object_t* stream_,
                                       object structure) { GCTRIGGER1(structure); {
  {var object name = Car(TheStructure(structure)->structure_types);
  /* name = (car '(name_1 ... name_i-1 name_i)) = name_1. */
  pushSTACK(structure);
  pushSTACK(name);
...
/* -------- machine pointer --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_machine (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  /* #<ADDRESS #x...> */
...
/* -------- Frame-Pointer, Small-Read-Label, System --------
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_system (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  if (nullpSv(print_readably)) {
    if (!boundp(obj))
...
/* UP: prints read-label to stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_readlabel (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  CHECK_PRINT_READABLY(obj);
  /* #<READ-LABEL ...> */
  UNREADABLE_START;
  JUSTIFY_LAST(false);
...
{ GCTRIGGER1(obj); { pr_uint(stream_,STACK_item_count(uTheFramepointer(obj),
                                   (gcv_object_t*)STACK_start)); }}
local maygc void pr_framepointer (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  CHECK_PRINT_READABLY(obj);
  /* #<FRAME-POINTER N> */
...
local maygc void pr_record_ab (const gcv_object_t* stream_,
                               const gcv_object_t* obj_,
                               uintL index, uintL length) { GCTRIGGER(); {
...
/* a pr_routine_t version of pr_record_ab
 ...
 < stream - same stream
 can trigger GC */
local maygc void pr_record_ab_00 (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  pushSTACK(obj);
...
local maygc void pr_record_descr (const gcv_object_t* stream_, object obj,
                                  object name, bool readable, object slotlist) { GCTRIGGER3(obj,name,slotlist); {
  LEVEL_CHECK;
  {
    pushSTACK(obj);
    pushSTACK(name);
...
/* UP: prints an OtherRecord to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_orecord (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* -------- SUBRs, FSUBRs --------
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_subr (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* UP: prints FSUBR to Stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_fsubr (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* -------- Closures --------
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_closure (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* UP: prints compiled Closure to Stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_cclosure (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  /* query *PRINT-CLOSURE* : */
...
/* print compiled Closure in rereadable Form:
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_cclosure_lang (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  LEVEL_CHECK;
  {
    pushSTACK(obj);                    /* save Closure */
    {var gcv_object_t* obj_ = &STACK_0; /* and memorize, where it is */
...
/* UP: prints Closure-Codevector in #nY(...)-notation
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_cclosure_codevector (const gcv_object_t* stream_, object codevec) { GCTRIGGER1(codevec); {
  LEVEL_CHECK;
  {
...
/* -------- Streams --------
 ...
 < stream: Stream
 can trigger GC */
local maygc void pr_stream (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  CHECK_PRINT_READABLY(obj);
...
/* ---------------------- Top-Level-Call of the Printers -------------------
 ...
 < stream: stream
 can trigger GC */
global maygc void prin1 (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
...
/* UP: print Newline first, then print an object to stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void print (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  pushSTACK(obj);               /* save Object */
...
/* ----------------------- Helper-functions of the Printer -----------------
 ...
 can trigger GC */
local maygc void check_ostream (gcv_object_t* stream_) { GCTRIGGER(); {
...
/* UP: for WRITE-STRING and WRITE-LINE:
 ...
 can trigger GC */
local maygc void write_string_up (void) { GCTRIGGER(); {
