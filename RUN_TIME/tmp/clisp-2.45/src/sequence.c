...
/* UP: checks whether the given type is a valid sequence type name (otherwise
 ...
 can trigger GC */
local maygc object valid_type1 (gcv_object_t* type_) { GCTRIGGER(); {
...
local maygc object valid_type (gcv_object_t* type_) { GCTRIGGER(); {
...
/* UP: Copies a part of sequence into another sequence.
 ...
 can trigger GC */
local maygc void copy_seqpart_into (void) { GCTRIGGER(); {
...
local maygc void verify_return_value (gcv_object_t *retvalue,
                                      gcv_object_t *rettype) { GCTRIGGER(); {
...
global maygc Values coerce_sequence (object sequence, object result_type,
                                     bool error_p) { GCTRIGGER2(sequence,result_type); {
  pushSTACK(sequence);
...
local maygc Values seq_boolop (seq_boolop_fun* boolop_fun,
                               ...
                               uintC argcount,
                               object defolt) { GCTRIGGER1(defolt); {
...
/* UP: Prepares a sequence filtering operation.
 ...
 < Stack layout:
      ...
      l [STACK].
 can trigger GC */
local maygc void seq_prepare_filterop (gcv_object_t* stackptr) { GCTRIGGER(); {
  /* COUNT argument must be NIL or an integer >= 0: */
  test_count_arg();
...
local maygc Values seq_filterop (gcv_object_t* stackptr, funarg_t* pcall_test,
                                 help_function help_fun) { GCTRIGGER(); {
  pushSTACK(*(stackptr STACKop 0)); /* sequence */
...
/* UP: Helper function for the REMOVE functions
 ...
 < result: New sequence
 can trigger GC */
local maygc object remove_help (gcv_object_t* stackptr, uintV bvl, uintV dl) { GCTRIGGER(); {
...
/* UP: Helper function for the DELETE functions.
 ...
 can trigger GC */
local maygc object delete_help (gcv_object_t* stackptr, uintV bvl, uintV dl) { GCTRIGGER(); {
  /* dl=0 -> return unchanged sequence: */
  if (dl==0)
    return *(stackptr STACKop 0);
...
local maygc Values seq_duplicates (help_function help_fun) { GCTRIGGER(); {
...
/* UP: Executes a FIND operation.
 ...
 can trigger GC */
local maygc Values find_op (gcv_object_t* stackptr, funarg_t* pcall_test) { GCTRIGGER(); {
  pushSTACK(*(stackptr STACKop 0)); /* sequence */
...
/* UP: Executes a POSITION operation.
 ...
 < mv_space/mv_count: return values
 can trigger GC */
local maygc Values position_op (gcv_object_t* stackptr, funarg_t* pcall_test) { GCTRIGGER(); {
  pushSTACK(*(stackptr STACKop 0)); /* sequence */
  /* Stack layout: ..., typdescr, sequence. */
  if (!(nullp(*(stackptr STACKop -1)))) { /* check from-end */
    /* from-end is given */
...
/* UP: Executes a COUNT operation.
 ...
 < mv_space/mv_count: return values
 can trigger GC */
local maygc Values count_op (gcv_object_t* stackptr, funarg_t* pcall_test) { GCTRIGGER(); {
  pushSTACK(*(stackptr STACKop 0)); /* sequence */
...
/* UP for SORT, STABLE-SORT and MERGE:
 ...
 can trigger GC */
local maygc void merge (gcv_object_t* stackptr) { GCTRIGGER(); {
  while (1) {
...
local maygc object sort_part (object pointer_left, object k,
                              gcv_object_t* stackptr) { GCTRIGGER2(pointer_left,k); {
  if (eq(k,Fixnum_1)) {
...
/* UP for SORT and STABLE-SORT: Sorts a part of a sequence.
 ...
 < mv_space/mv_count: Values
 can trigger GC */
local maygc Values stable_sort (void) { GCTRIGGER(); {
  /* Stack layout: sequence, predicate, key, start, end. */
  /* Check sequence: */
