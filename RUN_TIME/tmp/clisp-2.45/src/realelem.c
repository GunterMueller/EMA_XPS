...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_to_LF(x,len,true) : F_to_LF(x,len)); }}
...
/* Warns, if floating-point and rational numbers are combined, and the
 ...
 can trigger GC */
local maygc void warn_floating_point_rational_contagion (void) { GCTRIGGER(); {
...
/* When an operation combines a rational and a floating-point number and the
 ...
 can trigger GC */
local maygc object RA_F_exact_contagion_R (object result, object float_argument) { GCTRIGGER2(result,float_argument); {
...
/* I_float_F(x) converts an Integer x into a Float and rounds.
 ...
 < result: (float x)
 can trigger GC */
local maygc object I_float_F (object x) { GCTRIGGER1(x); {
...
/* RA_float_F(x) converts a rational number x into a Float and rounds.
 ...
 < result: (float x)
 can trigger GC */
local maygc object RA_float_F (object x) { GCTRIGGER1(x); {
  defaultfloatcase(S(default_float_format),Fixnum_0,
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_float_F(x) : x); }}
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_minus_RA(x) : F_minus_F(x)); }}

/* R_abs_R(x) => (abs x), with x being a real number.
 can trigger GC */
local maygc object R_abs_R (object x)
{ GCTRIGGER1(x); { return (R_minusp(x) ? R_minus_R(x) : x); }} /* x<0 -> (- x), x>=0 -> x */
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_square_RA(x) : F_square_F(x)); }}
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_div_RA(x) : F_div_F(x)); }}
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? RA_1_plus_RA(x) : R_R_plus_R(x,Fixnum_1)); }}

/* R_minus1_plus_R(x) => (1- x), with x being a real number.
 can trigger GC */
local maygc object R_minus1_plus_R (object x)
{ GCTRIGGER1(x); { return R_rationalp(x) ? RA_minus1_plus_RA(x) : R_R_plus_R(x,Fixnum_minus1); }}
...
{ GCTRIGGER1(x); { F_integer_decode_float_I_I_I(x);
  /* stack layout: m, e, s. */
...
{ GCTRIGGER1(x); { return (R_rationalp(x) ? x : F_rational_RA(x)); }}
...
{ GCTRIGGER2(x,y); { pushSTACK(x); pushSTACK(y); /* save both */
 {{var object erg =
...
{ GCTRIGGER1(x); { if (R_rationalp(x)) { return x; } /* x rational -> x as result. */
