...
/* UP: Creates a new empty symbol-table.
 ...
 can trigger GC */
local maygc object make_symtab (uintL size) { GCTRIGGER(); {
...
/* takes a Cons from free-conses or returns a fresh one.
 ...
 can trigger GC */
local maygc object new_cons (void) { GCTRIGGER(); {
...
/* inserts an additional symbol into the new table.
 ...
 can trigger GC */
local maygc void newinsert (object sym, uintL size) { GCTRIGGER1(sym); {
...
/* UP: Inserts a given symbol into the symbol-table (destructively).
 ...
 call only, if BREAK_SEM_2 is set
 can trigger GC */
local maygc object symtab_insert (object sym, object symtab) { GCTRIGGER2(sym,symtab); {
...
local maygc object make_package (object name, object nicknames,
                                 bool case_sensitive_p, bool case_inverted_p) { GCTRIGGER2(name,nicknames); {
  set_break_sem_2();
...
/* UP: Adds a symbol to the shadowing-list of a package, that does not yet
 ...
 can trigger GC */
local maygc void shadowing_insert (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
/* raise a continuable error when func(obj) was called on a locked package pack
 ...
 can trigger GC */
local maygc void cerror_package_locked (object func, object pack, object obj) { GCTRIGGER3(func,pack,obj); {
...
/* UP: Inserts a symbol into a package, that has no symbol of the same name yet.
 ...
 can trigger GC */
local maygc void make_present (object sym, object pack) { GCTRIGGER2(sym,pack); {
...
/* UP: Interns a symbol with a given printname in a package.
 ...
 < result: 0, if not found, but newly created
           ...
           3, if available as internal symbol
 can trigger GC */
global maygc uintBWL intern (object string, bool invert, object pack, object* sym_) { GCTRIGGER2(string,pack); {
  {
...
/* UP: Interns a symbol of given printname into the keyword-package.
 ...
 can trigger GC */
global maygc object intern_keyword (object string) { GCTRIGGER1(string); {
  {var object sym;
...
/* UP: Imports a symbol into a package and turns it into a shadowing-symbol.
 ...
 < pack: package, EQ to the old one
 can trigger GC */
local maygc void shadowing_import (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
/* UP: Shadows in a package all symbols accessible from other packages
 ...
 can trigger GC */
local maygc void do_shadow (const gcv_object_t* sym_, bool invert, const gcv_object_t* pack_) { GCTRIGGER(); {
...
local maygc void shadow (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
  do_shadow(sym_,false,pack_);
}}
local maygc void cs_shadow (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
/* UP: Removes a symbol from the set of present symbols of a package
 ...
 can trigger GC */
local maygc object unintern (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
local maygc bool query_intern_conflict (object pack, object sym, object other,
                                        int dialog_type) { GCTRIGGER3(pack,sym,other); {
...
/* UP: Imports a symbol into a package and does conflict resolution
 ...
 < pack: package, EQ to the old
 can trigger GC */
global maygc void import (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
  {var object sym = *sym_;
  {var object pack = *pack_;
  {var object string = Symbol_name(sym);
  {var object othersym;
...
/* UP: Sets a symbol back from external to internal status in
 ...
 < pack: package, EQ to the old
 can trigger GC */
local maygc void unexport (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
/* UP: Sets a present symbol into external status.
 ...
 can trigger GC */
local maygc void make_external (object sym, object pack) { GCTRIGGER2(sym,pack); {
  if (symtab_find(sym,ThePackage(pack)->pack_external_symbols))
...
/* UP: Exports a symbol from a package
 ...
 < pack: package, EQ to the old
 can trigger GC */
global maygc void export (const gcv_object_t* sym_, const gcv_object_t* pack_) { GCTRIGGER(); {
...
/* UP: Applies a function to all symbols in a symboltable.
 ...
 > symtab: symboltable
 can trigger GC */
local maygc void map_symtab (object fun, object symtab) { GCTRIGGER2(fun,symtab); {
...
local maygc void map_symtab_c (one_sym_function_t* fun, void* data, object symtab) { GCTRIGGER1(symtab); {
  pushSTACK(Symtab_table(symtab)); /* table vector */
  /* number of entries */
  {var uintL size = posfixnum_to_V(Symtab_size(symtab));
  {var gcv_object_t* offset = 0; /* offset = sizeof(gcv_object_t)*index */
...
local maygc void use_package (object packlist, object pack) { GCTRIGGER2(packlist,pack); {
...
/* UP: Auxiliary function for use_package:
 ...
 can trigger GC */
local maygc void use_package_aux (void* data, object sym) { GCTRIGGER1(sym); {
...
/* UP: Effectuates, that a given package is not USE-ed anymore
 ...
 can trigger GC */
local maygc void unuse_1package (object pack, object qpack) { GCTRIGGER2(pack,qpack); {
...
/* UP: Effectuates, that a list of given packages is not USE-ed anymore
 ...
 can trigger GC */
local maygc void unuse_package (object packlist, object pack) { GCTRIGGER2(packlist,pack); {
  pushSTACK(pack);
  pushSTACK(packlist);
  set_break_sem_3();
  /* traverse packlist: */
...
/* UP: returns the current package
 ...
 can trigger GC */
global maygc object get_current_package (void) { GCTRIGGER(); {
...
/* UP: checks a package-argument.
 ...
 can trigger GC */
local maygc object test_package_arg (object obj) { GCTRIGGER1(obj); {
 restart_package_arg:
...
/* UP: checks name and nicknames -
 ...
 can trigger GC */
local maygc void test_names_args (void) { GCTRIGGER(); {
...
global maygc void symbol_value_check_lock (object caller, object symbol) { GCTRIGGER2(caller,symbol); {
...
/* UP: check the last argument &optional (pack *package*) of
 ...
 can trigger GC */
local maygc void test_optional_package_arg (void) { GCTRIGGER(); {
  {var object pack = STACK_0;
  if (!boundp(pack)) {
...
/* UP: Check of the arguments of INTERN and FIND-SYMBOL.
 test_intern_args()
 can trigger GC */
local maygc void test_intern_args (void) { GCTRIGGER(); {
...
local maygc Values do_intern (bool invert) { GCTRIGGER(); {
...
local maygc Values do_find_symbol (bool invert) { GCTRIGGER(); {
  test_intern_args(); /* test arguments */
  {var object pack = popSTACK();
  {var object string = popSTACK();
 #if !defined(VALUE1_EXTRA)
...
local maygc Values apply_symbols (sym_pack_function_t* fun) { GCTRIGGER(); {
...
/* UP: Preparation of the arguments of USE-PACKAGE and UNUSE-PACKAGE.
 ...
 can trigger GC */
local maygc void prepare_use_package (void) { GCTRIGGER(); {
...
/* UP: Corrects a package(nick)name.
 ...
 can trigger GC */
local maygc object correct_packname (object name, bool nickname_p) { GCTRIGGER1(name); {
  {var object pack;
...
/* UP for MAKE-PACKAGE and %IN-PACKAGE:
 ...
 can trigger GC */
local maygc void in_make_package (bool case_inverted) { GCTRIGGER(); {
...
/* UP: Auxiliary function for DELETE-PACKAGE:
 ...
 can trigger GC */
local maygc void delete_package_aux (void* data, object sym) { GCTRIGGER1(sym); {
...
local maygc Values do_find_all_symbols (bool invert) { GCTRIGGER(); {
...
/* UP: Auxiliary function for map_symbols:
 ...
 can trigger GC */
local maygc void map_symbols_aux (void* data, object sym) { GCTRIGGER1(sym); {
  {var gcv_object_t* localptr = (gcv_object_t*)data;
...
/* UP: initialize the package list
 init_packages();
 can trigger GC */
global maygc void init_packages (void) { GCTRIGGER(); {
