...
/* Warns, if Floats of different type are combined.
 ...
 can trigger GC */
local maygc void warn_floating_point_contagion (void) { GCTRIGGER(); {
...
/* F_abs_F(x) returns (abs x), with x being a Float.
 can trigger GC */
local maygc object F_abs_F (object x) { GCTRIGGER1(x); {
...
{ GCTRIGGER1(x); { /* method: x>=0 -> result 1.0; x<0 -> result -1.0 */
  floatcase(x, { /* x SF */
...
{ GCTRIGGER1(x); { /* method: if x=0.0, result 0, else (float-digits x). */
  floatcase(x,{
    if (SF_zerop(x))
      return Fixnum_0;
    return fixnum(SF_mant_len+1); /* Fixnum 17 */
