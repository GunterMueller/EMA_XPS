...
/* UP: Copies a list
 ...
 can trigger GC */
global maygc object copy_list (object old_list) { GCTRIGGER1(old_list); {
...
/* UP: Reverses a list by copying
 ...
 can trigger GC */
global maygc object reverse (object list) { GCTRIGGER1(list); {
...
/* UP: Constructs a list with exactly len elements.
 ...
 can trigger GC */
global maygc object make_list (uintL len) { GCTRIGGER(); {
  pushSTACK(NIL);
  while (len--) {
...
local maygc bool tree_equal (const gcv_object_t* stackptr, funarg_t* pcall_test,
                             object arg1, object arg2) { GCTRIGGER2(arg1,arg2); {
 start:
  if (atomp(arg1))
    if (atomp(arg2))
...
/* Finds the length of a possibly circular or dotted list.
 ...
 < dotted: if non-circular, the last atom, i.e., the indicator whether the list
           is dotted
 can trigger GC */
global maygc object list_length (object list, object *dottedp) { GCTRIGGER1(list); {
...
/* We cannot have lists longer than 1<<32 for RAM reasons
 ...
 can trigger GC */
local maygc uintL get_integer_truncate (object number) { GCTRIGGER1(number); {
...
/* Get a replacement for the circular list
 can trigger GC */
local maygc object replace_circular_list (object list) { GCTRIGGER1(list); {
...
global maygc object listof (uintC len) { GCTRIGGER(); {
...
/* UP: Copies an A-list
  ...
  can trigger GC */
local maygc object copy_alist (object alist) { GCTRIGGER1(alist); {
  /* Algorithm:
     Instead of
...
/* check that the argument is a non-circular list and return its length
 can trigger GC */
local inline maygc uintL check_list_length (gcv_object_t *list_) { GCTRIGGER(); {
  while(1) {
...
/* check_cons(obj)
 ...
 can trigger GC */
local maygc object check_cons_replacement (object obj) { GCTRIGGER1(obj); {
  do {
    pushSTACK(NIL);               /* no PLACE */
...
local inline maygc object check_cons (object obj) { GCTRIGGER1(obj); {
  if (!consp(obj))
...
local maygc object subst (object tree, gcv_object_t* stackptr,
                          funarg_t* pcall_test) { GCTRIGGER1(tree); {
...
/* UP: Replaces in tree all x by its A-LIST representation (by ASSOC):
 ...
 < result: (evtl. newer) Tree
 can trigger GC */
local maygc object sublis (object tree, gcv_object_t* stackptr) { GCTRIGGER1(tree); {
...
/* UP: Replaces in tree all x by its A-LIST representation (by ASSOC):
 ...
 < result: same Tree CAR
 can trigger GC */
local maygc object nsublis (object tree, gcv_object_t* stackptr) { GCTRIGGER1(tree); {
  /* First calculate (KEY tree) and call ASSOC: */
  pushSTACK(tree); /* save tree */
  funcall_key(*(stackptr STACKop -1),tree); /* (KEY tree) */
  *(stackptr STACKop -2) = value1; /* save for sublis_assoc */
...
local maygc object member (object list, gcv_object_t* stackptr,
                           funarg_t* pcall_test) { GCTRIGGER1(list); {
  while (!endp(list)) {
...
local maygc object assoc (object alist, gcv_object_t* stackptr,
                          funarg_t* pcall_test) { GCTRIGGER1(alist); {
 start:
