...
#if (defined(UNIX)) && (defined(HAVE_SYS_UTSNAME_H))
}
#endif
#line 167
}
...
/* push the (VAR . VALUE) on the STACK
 can trigger GC */
local inline maygc char* push_envar (char *env) { GCTRIGGER(); {
  char *ep = env;
  while ((*ep != 0) && (*ep != '=')) ep++;
...
/* === a module facility to map CPP constants to Lisp symbols. ===
 ...
 may trigger GC */
local maygc object map_to_alist (const c_lisp_map_t *map) { GCTRIGGER(); {
  unsigned int index;
...
global maygc long map_lisp_to_c (object obj, const c_lisp_map_t *map) { GCTRIGGER1(obj); {
  unsigned int index;
...
/* C number ---> Lisp symbol
 may trigger GC -- on error only */
global maygc object map_c_to_lisp (long val, const c_lisp_map_t *map) { GCTRIGGER(); {
  unsigned int index;
...
/* C number ---> list of Lisp symbols, each denoting a bit
 may trigger GC */
global maygc object map_c_to_list (long val, const c_lisp_map_t *map) { GCTRIGGER(); {
...
/* list of Lisp symbols, each denoting a bit ---> C number
 may trigger GC -- on error only */
global maygc long map_list_to_c (object obj, const c_lisp_map_t *map) { GCTRIGGER1(obj); {
  if (listp(obj)) {
    long ret = 0;
    pushSTACK(obj);
...
global maygc void push_string_array (char **arr) { GCTRIGGER(); {
  int count = 0;
  for (; *arr; count++, arr++)
...
{ GCTRIGGER(); { return asciz ? asciz_to_string(asciz,O(misc_encoding)) : NIL; }}
