...
/* error-message, if a symbol has no value.
 ...
 < value1: bound value
 can trigger GC */
local maygc void check_global_symbol_value (object symbol) { GCTRIGGER1(symbol); {
...
/* UP: just like GET-FUNNAME-SYMBOL (see init.lisp),
 ...
 can trigger GC */
local maygc object funname_to_symbol (object symbol) { GCTRIGGER1(symbol); {
  if (!funnamep(symbol))
...
/* UP: Check the body of a SETQ- or PSETQ-form.
 ...
 can trigger GC */
local maygc bool check_setq_body (object caller) { GCTRIGGER1(caller); {
...
/* call parse_dd() and maybe complain about doc-string
 ...
 > body: whole Body
 can trigger GC */
local maygc bool parse_doc_decl (object body, bool permit_doc_string) { GCTRIGGER1(body); {
  pushSTACK(body);
...
{ GCTRIGGER(); { /* execute (SYS::COMPILE-FORM form venv fenv benv genv denv) :
...
/* signal a correctable error for a broken LET variable spec
 can trigger GC */
local maygc object check_varspec (object varspec, object caller) { GCTRIGGER2(varspec,caller); {
  pushSTACK(NIL);     /* no PLACE */
...
/* UP: Finishes a FLET/MACROLET.
 ...
 < mv_count/mv_space: values
 can trigger GC */
local maygc Values finish_flet (gcv_object_t* top_of_frame, object body) { GCTRIGGER1(body); {
  {
...
local inline maygc void set_last_copy (object list) { GCTRIGGER1(list); {
  if (consp(list)) {
    pushSTACK(list);
...
/* Checks an optional macroexpansion-environment in STACK_0.
 ...
 can trigger GC */
local maygc void test_env (void) { GCTRIGGER(); {
