...
/* Convert the IP address from C format to Lisp
 ...
 can trigger GC */
global maygc object addr_to_string (short type, char *addr) { GCTRIGGER(); {
...
/* Convert the IP address from C format to Lisp
 ...
 for syscalls & rawsock modules
 can trigger GC */
global maygc object string_to_addr (const char* name) { GCTRIGGER(); {
  object ret;
  begin_system_call();
...
#if defined(hpux /* this is disgusting */)
}
#endif
#if defined(hpux /* this is disgusting */)
}
