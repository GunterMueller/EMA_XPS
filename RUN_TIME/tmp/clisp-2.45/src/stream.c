...
local maygc void wr_ch_array_dummy (const gcv_object_t* stream_,
                                    const gcv_object_t* chararray_,
                                    uintL start, uintL len) { GCTRIGGER(); {
  {var uintL end = start + len;
  {var uintL index = start;
...
/* Function: Returns the last character read (and not yet unread) from a stream.
 ...
 can trigger GC */
global maygc object stream_get_lastchar (object stream) { GCTRIGGER1(stream); {
...
/* Function: Sets the last character read (and not yet unread) of a stream.
 ...
 can trigger GC */
local maygc void stream_set_lastchar (object stream, object ch) { GCTRIGGER2(stream,ch); {
...
/* Reads a Byte from a Stream.
 ...
 < result: read Integer (eof_value at EOF)
 can trigger GC */
global maygc object read_byte (object stream) { GCTRIGGER1(stream); {
  if (builtin_stream_p(stream)) {
...
global maygc uintL read_byte_array (const gcv_object_t* stream_,
                                    const gcv_object_t* bytearray_,
                                    uintL start, uintL len,
                                    perseverance_t persev) { GCTRIGGER(); {
  if (len==0)
    return 0;
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* Writes a Byte to a Stream.
 ...
 can trigger GC */
global maygc void write_byte (object stream, object byte) { GCTRIGGER2(stream,byte); {
  if (builtin_stream_p(stream)) {
...
global maygc uintL write_byte_array (const gcv_object_t* stream_,
                                     const gcv_object_t* bytearray_,
                                     uintL start, uintL len, perseverance_t persev) { GCTRIGGER(); {
  if (len==0)
    return 0;
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* Reads a Character from a Stream.
 ...
 < result: read Character (eof_value at EOF)
 can trigger GC */
global maygc object read_char (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* pushes the last read Character back to the Stream.
 ...
 < stream: Stream
 can trigger GC */
global maygc void unread_char (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* Reads a Character from a Stream, without consuming it.
 ...
 < result: read Character (eof_value at EOF)
 can trigger GC */
global maygc object peek_char (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
    if (!(TheStream(stream)->strmflags & strmflags_unread_B)) /* Char after UNREAD ? */
...
/* writes a Character to a Stream.
 ...
 < stream: Stream
 can trigger GC */
global maygc void write_char (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* Outputs a real newline if an elastic newline is pending on the stream.
 ...
 < stream: Stream
 can trigger GC */
local maygc void harden_elastic_newline (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
...
/* Error message and get a replacement for an argument
 ...
 can trigger GC */
local maygc object get_streamtype_replacement (object obj, object type) { GCTRIGGER2(obj,type); {
...
/* UP: checks, if Arguments are Streams.
 ...
 can trigger GC */
local maygc void check_stream_args (gcv_object_t* args_pointer, uintC argcount) { GCTRIGGER(); {
  while (argcount--) {
...
local maygc object rd_by_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
local maygc void wr_by_synonym (object stream, object obj) { GCTRIGGER2(stream,obj); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
local maygc object rd_ch_synonym (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  {var object stream = *stream_;
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
  pushSTACK(get_synonym_stream(symbol));
...
local maygc object pk_ch_synonym (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  {var object stream = *stream_;
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
  pushSTACK(get_synonym_stream(symbol));
...
local maygc void wr_ch_synonym (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  check_SP(); check_STACK();
  {var object stream = *stream_;
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
  pushSTACK(get_synonym_stream(symbol));
...
#else
  local maygc void close_synonym (object stream, uintB abort) { GCTRIGGER1(stream); {
    check_SP(); check_STACK();
    {var object symbol = TheStream(stream)->strm_synonym_symbol;
    {var int argcount = 1;
    pushSTACK(get_synonym_stream(symbol));
...
/* Determines, if a character is available on the Synonym-Stream.
 ...
 can trigger GC */
local maygc signean listen_char_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* UP: Deletes already entered interactive Input from a Synonym-Stream.
 ...
 can trigger GC */
local maygc bool clear_input_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* Determines, if a Byte is available on a Synonym-Stream.
 ...
 can trigger GC */
local maygc signean listen_byte_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* UP: bring pending Output of a Synonym-Stream to the destination.
 ...
 > stream: Synonym-Stream
 can trigger GC */
local maygc void finish_output_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* UP: bring pending Output of a Synonym-Stream to the destination.
 ...
 > stream: Synonym-Stream
 can trigger GC */
local maygc void force_output_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* UP: delete the pending Output of a Synonym-Stream.
 ...
 > stream: Synonym-Stream
 can trigger GC */
local maygc void clear_output_synonym (object stream) { GCTRIGGER1(stream); {
  check_SP();
  {var object symbol = TheStream(stream)->strm_synonym_symbol;
...
/* Returns a Synonym-Stream for a Symbol.
 ...
 can trigger GC */
local maygc object make_synonym_stream (object symbol) { GCTRIGGER1(symbol); {
...
local maygc void wr_by_broad (object stream, object obj) { GCTRIGGER2(stream,obj); {
  check_SP(); check_STACK();
  pushSTACK(obj);
  { /* list of streams */
...
local maygc void wr_ch_broad (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  check_SP(); check_STACK();
  pushSTACK(obj);
  pushSTACK(NIL); /* dummy */
  pushSTACK(TheStream(*stream_)->strm_broad_list); /* list of streams */
...
/* UP: Moves the pending Output of a Broadcast-Stream to the destination.
 ...
 can trigger GC */
local maygc void finish_output_broad (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  {var object streamlist = TheStream(stream)->strm_broad_list;
...
/* UP: Moves the pending Output of a Broadcast-Stream to the destination.
 ...
 > stream: Broadcast-Stream
 can trigger GC */
local maygc void force_output_broad (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  {var object streamlist = TheStream(stream)->strm_broad_list;
  /* treat each Stream from the List separately: */
  while (consp(streamlist)) {
...
/* UP: Deletes the pending Output of a Broadcast-Stream.
 ...
 > stream: Broadcast-Stream
 can trigger GC */
local maygc void clear_output_broad (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  {var object streamlist = TheStream(stream)->strm_broad_list;
  /* treat each Stream from the List separately: */
  while (consp(streamlist)) {
...
/* Returns a Broadcast-Stream for a list of Streams.
 ...
 can trigger GC */
local maygc object make_broadcast_stream (object list) { GCTRIGGER1(list); {
...
/* returns a Broadcast-Stream for Stream stream.
 ...
 < result : Broadcast-Stream
 can trigger GC */
global maygc object make_broadcast1_stream (object oldstream) { GCTRIGGER1(oldstream); {
  pushSTACK(oldstream);
...
local maygc object rd_by_concat (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  pushSTACK(stream);
...
local maygc object rd_ch_concat (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
...
local maygc object pk_ch_concat (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  {var object streamlist = TheStream(*stream_)->strm_concat_list;
  while (consp(streamlist)) {
    pushSTACK(Car(streamlist));
...
/* Determines, if a character is available on the Concatenated-Stream.
 ...
 < result:   ls_avail if a character is available,
             ...
             ls_wait  if no character is available, but not because of EOF
 can trigger GC */
local maygc signean listen_char_concat (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
  {var object streamlist = TheStream(stream)->strm_concat_list;
  {var signean result;
  while (consp(streamlist)) {
...
/* UP: Deletes already entered interactive Input from a
 ...
 < result: true if Input was deleted
 can trigger GC */
local maygc bool clear_input_concat (object stream) { GCTRIGGER1(stream); {
...
/* Determines, if a Byte is available on the Concatenated-Stream.
 ...
 < result:   ls_avail if a byte is available,
             ...
             ls_wait  if no byte is available, but not because of EOF
 can trigger GC */
local maygc signean listen_byte_concat (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
  {var object streamlist = TheStream(stream)->strm_concat_list;
  {var signean result;
  while (consp(streamlist)) {
...
/* Returns a Concatenated-Stream for a list of Stream.
 ...
 Thereby the List list is destroyed.
 can trigger GC */
local maygc object make_concatenated_stream (object list) { GCTRIGGER1(list); {
  pushSTACK(list); /* save list */
...
local maygc void wr_by_twoway (object stream, object obj) { GCTRIGGER2(stream,obj); {
  check_SP();
...
local maygc void wr_ch_twoway (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  check_SP(); check_STACK();
  pushSTACK(TheStream(*stream_)->strm_twoway_output);
  write_char(&STACK_0,obj);
  skipSTACK(1);
...
/* Determines, if a Character is available on a Two-Way- or Echo-Stream.
 ...
 < result:   ls_avail if a character is available,
             ...
             ls_wait  if no character is available, but not because of EOF
 can trigger GC */
local maygc signean listen_char_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
/* UP: Deletes already entered interactive Input from a Two-Way-
 ...
 < result: true if Input was deleted
 can trigger GC */
local maygc bool clear_input_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
/* Determines, if a Byte is available on a Two-Way- or Echo-Stream.
 ...
 < result:   ls_avail if a byte is available,
             ...
             ls_wait  if no byte is available, but not because of EOF
 can trigger GC */
local maygc signean listen_byte_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
/* UP: Moves the pending Output of a Two-Way- or Echo-Stream to the destination.
 ...
 > stream: Two-Way- or Echo-Stream
 can trigger GC */
local maygc void finish_output_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
/* UP: Moves the pending Output of a Two-Way- or Echo-Stream to the destination.
 ...
 > stream: Two-Way- or Echo-Stream
 can trigger GC */
local maygc void force_output_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
/* UP: Deletes the pending Output of a Two-Way- or Echo-Stream.
 ...
 > stream: Two-Way- or Echo-Stream
 can trigger GC */
local maygc void clear_output_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
local maygc object rd_by_twoway (object stream) { GCTRIGGER1(stream); {
  check_SP();
...
local maygc object rd_ch_twoway (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  pushSTACK(TheStream(*stream_)->strm_twoway_input);
  {var object result = read_char(&STACK_0);
  skipSTACK(1);
...
local maygc object pk_ch_twoway (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  pushSTACK(TheStream(*stream_)->strm_twoway_input);
  {var object result = peek_char(&STACK_0);
  skipSTACK(1);
...
/* Reads a line of characters from a two-way-stream.
 ...
 < result: true if EOF was seen before newline, else false
 can trigger GC */
local maygc bool read_line_twoway (object stream, const gcv_object_t* buffer_) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  pushSTACK(TheStream(stream)->strm_twoway_input);
  {var bool eofp = read_line(&STACK_0,buffer_);
  skipSTACK(1);
...
global maygc object make_twoway_stream (object input_stream,
                                        object output_stream) { GCTRIGGER2(input_stream,output_stream); {
...
local maygc object rd_by_echo (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  pushSTACK(stream);
...
local maygc object rd_ch_echo (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP(); check_STACK();
  pushSTACK(TheStream(*stream_)->strm_twoway_input);
...
{ GCTRIGGER2(input_stream,output_stream); {
  pushSTACK(input_stream); pushSTACK(output_stream); /* save Streams */
  {var uintB flags = strmflags_rdwr_B
    | (TheStream(input_stream)->strmflags & strmflags_immut_B);
  {var object stream = /* new Stream, all Operations allowed */
...
local maygc object rd_ch_str_in (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
...
/* Closes a String-Input-Stream.
 ...
 > abort: flag: non-0 => ignore errors */
local maygc void close_str_in (object stream, uintB abort) { GCTRIGGER1(stream); {
...
/* Determines, if a character is available on a String-Input-Stream.
 ...
 < result:   ls_avail if a character is available,
             ...
             ls_wait  if no character is available, but not because of EOF
 can trigger GC */
local maygc signean listen_char_str_in (object stream) { GCTRIGGER1(stream); {
  {var uintV index = posfixnum_to_V(TheStream(stream)->strm_str_in_index);
  {var uintV endindex = posfixnum_to_V(TheStream(stream)->strm_str_in_endindex);
  if (index >= endindex)
...
local maygc void wr_ch_str_out (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
...
/* Returns a String-Output-Stream.
 ...
 can trigger GC */
global maygc object make_string_output_stream (void) { GCTRIGGER(); {
...
/* UP: Returns the collected stuff from a String-Output-Stream.
 ...
 can trigger GC */
global maygc object get_output_stream_string (const gcv_object_t* stream_) { GCTRIGGER(); {
...
local maygc void wr_ch_str_push (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  check_wr_char(stream,ch);
  /* push Character in the String: */
...
local maygc void wr_ch_pphelp (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  check_wr_char(stream,ch);
...
/* UP: Returns a Pretty-Printer-Auxiliary-Stream.
 make_pphelp_stream()
 can trigger GC */
global maygc object make_pphelp_stream (void) { GCTRIGGER(); {
...
local maygc object rd_ch_buff_in (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
...
/* Closes a Buffered-Input-Stream.
 ...
 > abort: flag: non-0 => ignore errors */
local maygc void close_buff_in (object stream, uintB abort) { GCTRIGGER1(stream); {
...
/* Determines, if a character is available on a Buffered-Input-Stream.
 ...
 < result:   ls_avail if a character is available,
             ...
             ls_wait  if no character is available, but not because of EOF
 can trigger GC */
local maygc signean listen_char_buff_in (object stream) { GCTRIGGER1(stream); {
  {var uintV index = posfixnum_to_V(TheStream(stream)->strm_buff_in_index);
...
/* UP: Deletes already entered interactive Input from a Buffered-Input-Stream.
 ...
 < result: true if Input was deleted
 can trigger GC */
local maygc bool clear_input_buff_in (object stream) { GCTRIGGER1(stream); {
...
/* UP: Moves the pending Output of a Buffered-Output-Stream to the destination.
 ...
 can trigger GC */
local maygc void finish_output_buff_out (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
...
/* UP: Deletes the pending Output of a Buffered-Output-Stream.
 ...
 > stream: Buffered-Output-Stream
 can trigger GC */
local maygc void clear_output_buff_out (object stream) { GCTRIGGER1(stream); {
  /* empty String by Fill-Pointer:=0 : */
  TheIarray(TheStream(stream)->strm_buff_out_string)->dims[1] = 0;
...
local maygc void wr_ch_buff_out (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  check_wr_char(stream,ch);
  /* push Character in the String: */
...
/* Closes a Buffered-Output-Stream.
 ...
 > abort: flag: non-0 => ignore errors
 can trigger GC */
local maygc void close_buff_out (object stream, uintB abort) { GCTRIGGER1(stream); {
...
local maygc object rd_ch_generic (const gcv_object_t* stream_) { GCTRIGGER(); {
...
local maygc object pk_ch_generic (const gcv_object_t* stream_) { GCTRIGGER(); {
  pushSTACK(*stream_); funcall(L(generic_stream_controller),1);
...
/* (LISTEN s) ==
 (IF (GENERIC-STREAM-READ-CHAR-WILL-HANG-P c)
   ...
   (IF (GENERIC-STREAM-PEEK-CHAR c)
     :INPUT-AVAILABLE
     :EOF)) */
local maygc signean listen_char_generic (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
...
local maygc bool clear_input_generic (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream); funcall(L(generic_stream_controller),1);
...
local maygc void wr_ch_generic (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
...
local maygc void wr_by_generic (object stream, object obj) { GCTRIGGER2(stream,obj); {
...
local maygc void close_generic (object stream, uintB abort) { GCTRIGGER1(stream); {
  pushSTACK(stream); funcall(L(generic_stream_controller),1);
...
/* UP: Returns a canonical representation for a :ELEMENT-TYPE.
 ...
 can trigger GC */
local maygc object canon_eltype (const decoded_el_t* decoded) { GCTRIGGER(); {
...
local maygc object bitbuff_iu_I (object bitbuffer, uintL bitsize,
                                 uintL bytesize) { GCTRIGGER1(bitbuffer); {
...
/* UP for READ-BYTE on File-Streams of Integers, Type u :
 ...
 can trigger GC */
local maygc object rd_by_iu_I (object stream, uintL bitsize, uintL bytesize) { GCTRIGGER1(stream); {
...
/* UP for READ-BYTE on File-Streams of Integers, Type s :
 ...
 can trigger GC */
local maygc object rd_by_is_I (object stream, uintL bitsize, uintL bytesize) { GCTRIGGER1(stream); {
...
local maygc void wr_by_ixu_sub (object stream, object obj,
                                wr_by_aux_ix* finisher) { GCTRIGGER2(stream,obj); {
...
#if (!(!defined(WIN32_NATIVE))) && (defined(WIN32_NATIVE))
}
#endif
#if (!(!defined(WIN32_NATIVE))) && (defined(WIN32_NATIVE))
}
...
local maygc void wr_by_iau_unbuffered (object stream, object obj) { GCTRIGGER2(stream,obj); {
  wr_by_ixu_sub(stream,obj,&wr_by_aux_ia_unbuffered);
}}

/* WRITE-BYTE - Pseudo-Function for File-Streams of Integers, Type as : */
local maygc void wr_by_ias_unbuffered (object stream, object obj) { GCTRIGGER2(stream,obj); {
  wr_by_ixs_sub(stream,obj,&wr_by_aux_ia_unbuffered);
}}

/* WRITE-BYTE - Pseudo-Function for Handle-Streams, Type au, bitsize = 8 : */
local maygc void wr_by_iau8_unbuffered (object stream, object obj) { GCTRIGGER2(stream,obj); {
  ASSERT_wr_int(stream,obj);
...
/* WRITE-CHAR - Pseudo-Function for Unbuffered-Channel-Streams: */
local maygc void wr_ch_unbuffered_mac (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  check_wr_char(stream,ch);
  {var chart c = char_code(ch); /* Code of the character */
  if (chareq(c,ascii(NL)))
...
/* WRITE-CHAR - Pseudo-Function for Unbuffered-Channel-Streams: */
local maygc void wr_ch_unbuffered_dos (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var object stream = *stream_;
  check_wr_char(stream,ch);
  {var chart c = char_code(ch); /* Code of the character */
...
/* UP: Move the pending Output of a Unbuffered-Channel-Stream to the destination.
 ...
 can trigger GC */
local maygc void finish_output_unbuffered (object stream) { GCTRIGGER1(stream); {
  oconv_unshift_output_unbuffered(stream);
...
/* UP: Move the pending Output of a Unbuffered-Channel-Stream to the destination.
 ...
 > stream: Handle-Stream
 can trigger GC */
local maygc void force_output_unbuffered (object stream) { GCTRIGGER1(stream); {
  oconv_unshift_output_unbuffered(stream);
...
/* UP: Delete the pending Output of a Unbuffered-Channel-Stream.
 ...
 > stream: Handle-Stream
 can trigger GC */
local maygc void clear_output_unbuffered (object stream) { GCTRIGGER1(stream); {
...
local maygc void wr_ch_buffered_unix (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  {var object stream = *stream_;
...
/* WRITE-CHAR - Pseudo-Function for File-Streams of Characters */
local maygc void wr_ch_buffered_mac (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  {var object stream = *stream_;
  check_wr_char(stream,obj);
  {var chart c = char_code(obj);
  if (chareq(c,ascii(NL)))
...
/* WRITE-CHAR - Pseudo-Function for File-Streams of Characters */
local maygc void wr_ch_buffered_dos (const gcv_object_t* stream_, object obj) { GCTRIGGER1(obj); {
  {var object stream = *stream_;
  check_wr_char(stream,obj);
  {var chart c = char_code(obj);
#ifdef UNICODE
...
local maygc object make_buffered_stream (uintB type, direction_t direction,
                                         const decoded_el_t* eltype,
                                         bool handle_regular,
                                         bool handle_blockpositioning) { GCTRIGGER(); {
  {var uintB flags = DIRECTION_FLAGS(direction) & ELTYPE_FLAFS(eltype);
...
/* UP: add a stream to the list of open streams O(open_files)
 ...
 <> stream
 can trigger GC */
local maygc object add_to_open_streams (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
  {var object new_cons = allocate_cons();
...
/* UP: Moves the pending Output of a File-Stream to the destination.
 ...
 can trigger GC */
local maygc void finish_output_buffered (object stream) { GCTRIGGER1(stream); {
...
/* UP: extends the List STACK_0 by one key-assignment.
 can trigger GC */
local maygc void add_keybinding (const char* cap, const key_event_t* event) { GCTRIGGER(); {
...
/* returns a Keyboard-Stream.
 ...
 can trigger GC */
local maygc object make_keyboard_stream (void) { GCTRIGGER(); {
 #if defined(UNIX)
  {
...
local maygc char** lisp_completion (char* text, int start, int end) { GCTRIGGER(); {
...
/* read a character from a terminal-stream. */
local maygc object rd_ch_terminal2 (const gcv_object_t* stream_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (eq(TheStream(stream)->strm_rd_ch_last,eof_value)) /* EOF already? */
    return eof_value;
...
/* Determines, if a character is available on a Terminal-Stream.
 ...
 < result:   ls_avail if a character is available,
             ls_eof   if EOF is reached,
             ls_wait  if no character is available, but not because of EOF */
local maygc signean listen_char_terminal2 (object stream) { GCTRIGGER1(stream); {
  if (eq(TheStream(stream)->strm_rd_ch_last,eof_value)) /* EOF already? */
    return ls_eof;
  if (posfixnum_to_V(TheStream(stream)->strm_terminal_index)
      < TheIarray(TheStream(stream)->strm_terminal_inbuff)->dims[1])
...
/* UP: Deletes already entered interactive Input from a Terminal-Stream.
 ...
 > stream: Terminal-Stream
 < result: true if Input was deleted, else false */
local maygc bool clear_input_terminal2 (object stream) { GCTRIGGER1(stream); {
...
/* UP: Write a character to a Terminal-Stream.
 ...
 > stream: Terminal-Stream
 > ch: character to be written */
local maygc void wr_ch_terminal3 (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
...
/* UP: Deletes the pending Output of a Terminal-Stream.
 ...
 > stream: Terminal-Stream
 can trigger GC */
local maygc void clear_output_terminal3 (object stream) { GCTRIGGER1(stream); {
 #if TERMINAL_OUTBUFFERED
...
/* Returns an interactive Terminal-Stream.
 can trigger GC */
local maygc object make_terminal_stream_ (void) { GCTRIGGER(); {
  begin_system_call();
...
/* Returns an interactive Terminal-Stream.
 can trigger GC */
local maygc object make_terminal_stream (void) { GCTRIGGER(); {
...
/* UP: write a character to a Window-Stream.
 ...
 > ch: character to be written */
local maygc void wr_ch_window (const gcv_object_t* stream_, object ch) { GCTRIGGER1(ch); {
  {var Handle handle = ConsoleHandleR(*stream_);
  {var COORD  pos    = ConsoleData(*stream_)->cursor_position;
  {var COORD  sz     = ConsoleData(*stream_)->console_size;
  {var uintW  attr   = attr_table[ConsoleData(*stream_)->attribute];
...
/* mkops_from_handles(pipe,process_id)
 ...
 Used in LAUNCH
 can trigger GC */
global maygc object mkops_from_handles (Handle opipe, int process_id) { GCTRIGGER(); {
  {var decoded_el_t eltype;
  {var signean buffered;
  /* Check and canonicalize the :BUFFERED argument: */
  buffered = test_buffered_arg(STACK_0);
...
/* mkips_from_handles(pipe,process_id)
 ...
 Used in LAUNCH
 can trigger GC */
global maygc object mkips_from_handles (Handle ipipe, int process_id) { GCTRIGGER(); {
  {var decoded_el_t eltype;
  {var signean buffered;
  /* Check and canonicalize the :BUFFERED argument: */
  buffered = test_buffered_arg(STACK_0);
...
/* The two following functions should
 ...
 can trigger GC */
local maygc void test_n_bytes_args (uintL* index_, uintL* count_) { GCTRIGGER(); {
...
global maygc struct timeval * sec_usec (object sec, object usec,
                                        struct timeval *tv) { GCTRIGGER2(sec,usec); {
  if (missingp(sec)) {
    return NULL;
  } else if (consp(sec)) {
...
local maygc object handle_isset (object socket, fd_set *readfds,
                                 fd_set *writefds, fd_set *errorfds) { GCTRIGGER1(socket); {
  {var object sock, ret;
  {var direction_t dir;
...
local maygc object handle_to_stream (Handle fd, object direction, object buff_p,
                                     object ext_fmt, object eltype) { GCTRIGGER4(direction,buff_p,ext_fmt,eltype); {
  {var direction_t dir;
...
/* Allocates the equivalent of the C stream stdin.
 can trigger GC */
local inline maygc object make_standard_input_file_stream (void) { GCTRIGGER(); {
...
/* Allocates the equivalent of the C stream stdout.
 can trigger GC */
local inline maygc object make_standard_output_file_stream (void) { GCTRIGGER(); {
  /* This uses the external-format :default, not O(terminal_encoding),
     because this stream is used when *terminal-io* is not interactive. */
...
/* Allocates the equivalent of the C stream stderr.
 can trigger GC */
local inline maygc object make_standard_error_file_stream (void) { GCTRIGGER(); {
  /* This uses the external-format :default, not O(terminal_encoding),
     because this stream is used when *terminal-io* is not interactive. */
...
/* It is important to allocate each of the standard_output_file_stream,
 ...
 can trigger GC */
local maygc object get_standard_input_file_stream (void) { GCTRIGGER(); {
...
/* Returns the equivalent of the C stream stdout.
 can trigger GC */
local maygc object get_standard_output_file_stream (void) { GCTRIGGER(); {
  #if defined(UNIX) || defined(WIN32_NATIVE)
...
/* Returns the equivalent of the C stream stderr.
 can trigger GC */
local maygc object get_standard_error_file_stream (void) { GCTRIGGER(); {
  #if 0
...
/* UP: Returns the default value for *terminal-io*.
 can trigger GC */
local maygc object make_terminal_io (void) { GCTRIGGER(); {
...
/* UP: Initializes the stream-variables.
 ...
 can trigger GC */
global maygc void init_streamvars (bool batch_p) { GCTRIGGER(); {
  #ifdef GNU_READLINE
  begin_call();
...
/* combine several stream-element-type into one, using OR
 ...
 can trigger GC */
local maygc object combine_stream_element_types (uintL numarg) { GCTRIGGER(); {
...
/* UP: reset the stream for the eltype and flush out the missing LF.
 ...
 can trigger GC */
local maygc object stream_reset_eltype (object stream, decoded_el_t* eltype_) { GCTRIGGER1(stream); {
  if (ChannelStream_buffered(stream)) {
...
global maygc void set_terminalstream_external_format (object stream,
                                                      object encoding) { GCTRIGGER2(stream,encoding); {
  if (builtin_stream_p(stream)
...
/* UP: Closes a list of open files - no errors!
 ...
 can trigger GC */
global maygc void close_some_files (object list) { GCTRIGGER1(list); {
  pushSTACK(NIL); /* dummy */
  pushSTACK(list); /* list */
  while (mconsp(STACK_0)) {
...
/* UP: Closes all open files.
 close_all_files();
 can trigger GC */
global maygc void close_all_files (void) { GCTRIGGER(); {
...
/* Reads a line of characters from a stream.
 ...
 < result: true if EOF was seen before newline, else false
 can trigger GC */
local inline maygc bool push_ch (object ch, object buffer) { GCTRIGGER2(ch,buffer); {
  if (!charp(ch)) {
...
global maygc bool read_line (const gcv_object_t* stream_,
                             const gcv_object_t* buffer_) { GCTRIGGER(); {
  {var object stream = *stream_;
  if (builtin_stream_p(stream)) {
    if (TheStream(stream)->strmflags & strmflags_unread_B) { /* Char after UNREAD ? */
...
/* UP: Determines, if a character is instantly available in the Stream stream.
 ...
 < result:   ls_avail if a character is available,
             ...
             ls_wait  if no character is available, but not because of EOF
 can trigger GC */
global maygc signean listen_char (object stream) { GCTRIGGER1(stream); {
  if (builtin_stream_p(stream)) {
    check_SP(); check_STACK();
    if (TheStream(stream)->strmflags & strmflags_unread_B) { /* Char after UNREAD ? */
...
/* UP: Deletes already entered interactive Input from a Stream stream.
 ...
 < result: true if Input was deleted
 can trigger GC */
global maygc bool clear_input (object stream) { GCTRIGGER1(stream); {
  check_SP(); check_STACK();
  pushSTACK(stream); /* save Stream */
  /* call type-specific Routine (can trigger GC). */
  if (builtin_stream_p(stream)) {
...
/* UP: Determines whether a stream has a byte immediately available.
 ...
 < result: ls_avail if a byte is available,
           ...
           ls_wait  if no byte is available, but not because of EOF
 can trigger GC */
global maygc signean listen_byte (object stream) { GCTRIGGER1(stream); {
  if (builtin_stream_p(stream)) {
...
/* UP: Move the pending Output of a Stream to the destination.
 ...
 > stream: Stream
 can trigger GC */
global maygc void finish_output (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
...
/* UP: Move the pending Output of a Stream to the destination.
 ...
 > stream: Stream
 can trigger GC */
global maygc void force_output (object stream) { GCTRIGGER1(stream); {
  pushSTACK(stream);
  harden_elastic_newline(&STACK_0);
  stream = popSTACK();
  if (builtin_stream_p(stream)) {
...
/* UP: Delete pending Output of a Stream.
 ...
 > stream: Stream
 can trigger GC */
global maygc void clear_output (object stream) { GCTRIGGER1(stream); {
...
/* UP: Returns the Line-Position of a Stream.
 ...
 can trigger GC */
global maygc object get_line_position (object stream) { GCTRIGGER1(stream); {
  check_SP();
 start:
  if (builtin_stream_p(stream))
    switch (TheStream(stream)->strmtype) {
...
/* Tests whether the stream has an elastic newline pending.
 ...
 can trigger GC */
local maygc bool elastic_newline_pending_p (object stream) { GCTRIGGER1(stream); {
  check_SP();
 start:
  if (builtin_stream_p(stream))
    switch (TheStream(stream)->strmtype) {
...
/* Writes a newline on a stream, if it is not already positioned at column 0.
 ...
 can trigger GC */
global maygc bool fresh_line (const gcv_object_t* stream_) { GCTRIGGER(); {
...
/* Writes a newline on a stream, delayed and nullified if the next character
 ...
 < stream: Stream
 can trigger GC */
global maygc void elastic_newline (const gcv_object_t* stream_) { GCTRIGGER(); {
  check_SP();
  {var object stream = *stream_;
 start:
  if (builtin_stream_p(stream))
...
/* UP: Check an endianness argument.
 ...
 can trigger GC */
local bool maygc check_endianness_arg (object arg) { GCTRIGGER1(arg); {
...
/* extract the OS file handle from the file stream
 ...
 for syscall module
 can trigger GC */
global maygc object open_file_stream_handle (object stream, Handle *fd) { GCTRIGGER1(stream); {
...
/* UP: Returns the current line number of a stream.
 ...
 can trigger GC */
global maygc object stream_line_number (object stream) { GCTRIGGER1(stream); {
  return (builtin_stream_p(stream)
...
/* Function: Returns true if a stream is a FASL stream:
 ...
 can trigger GC */
global maygc bool stream_get_fasl (object stream) { GCTRIGGER1(stream); {
  if (builtin_stream_p(stream)) {
...
/* Function: Changes the FASL state of a stream.
 ...
 can trigger GC */
global maygc void stream_set_fasl (object stream, bool value) { GCTRIGGER1(stream); {
  if (builtin_stream_p(stream)) {
    if (value)
