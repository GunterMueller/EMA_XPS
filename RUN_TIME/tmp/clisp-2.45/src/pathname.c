...
local maygc object common_case (object string) { GCTRIGGER1(string); {
...
/* UP: check an optional HOST argument
 ...
 can trigger GC */
local maygc object test_optional_host (object host, bool convert) { GCTRIGGER1(host); {
...
/* UP: check an optional HOST argument
 ...
 < result: valid host-component
 can trigger GC */
local maygc object test_optional_host (object host) { GCTRIGGER1(host); {
  if (!boundp(host) || eq(host,S(Kunspecific)))
...
/* checks a default-pathname.
 ...
 can trigger GC */
local maygc object test_default_pathname (object defaults) { GCTRIGGER1(defaults); {
...
/* <http://www.lisp.org/HyperSpec/Body/sec_19-2-3.html>:
 ...
 < result: a pathname derived from it, with *DEFAULT-PATHNAME-DEFAULTS* merged
           in.
 can trigger GC */
local maygc object merge_defaults (object pathname) { GCTRIGGER1(pathname); {
...
/* Parses the name/type/version part (if subdirp=false) or a subdir part
 ...
 can trigger GC */
local maygc object parse_logical_word (zustand* z, bool subdirp) { GCTRIGGER(); {
...
/* Attempt to parse a logical host name string, starting at a given state.
 ...
 can trigger GC */
local maygc object parse_logical_host_prefix (zustand* zp, object string) { GCTRIGGER1(string); {
...
/* CLHS for MAKE-PATHNAME: "Whenever a pathname is constructed the
 ...
 can trigger GC */
local maygc object simplify_directory (object dir) { GCTRIGGER1(dir); {
...
/* Parses a logical pathname.
 ...
 can trigger GC */
local maygc uintL parse_logical_pathnamestring (zustand z) { GCTRIGGER(); {
...
{ GCTRIGGER(); { /* make sure *PARSE-NAMESTRING-DOT-FILE* is valid */
...
/* auxiliary function for PARSE-NAMESTRING:
 ...
 can trigger GC */
local maygc void split_name_type (uintL skip) { GCTRIGGER(); {
  if (skip == 0) {
...
/* UP: Converts an object into a pathname.
 ...
 can trigger GC */
local maygc object coerce_xpathname (object obj) { GCTRIGGER1(obj); {
  if (xpathnamep(obj)) {
...
/* Converts obj to a pathname. If obj is a string, it is even converted to a
   logical pathname.
 can trigger GC */
local maygc object parse_as_logical (object obj) { GCTRIGGER1(obj); {
...
/* UP: Change an object into a non-logical pathname.
 ...
 can trigger GC */
local maygc object coerce_pathname (object obj) { GCTRIGGER1(obj); {
...
/* UP: Pushes substrings for STRING_CONCAT on the STACK, that together yield
 ...
 can trigger GC
 changes STACK */
local maygc uintC file_namestring_parts (object pathname) { GCTRIGGER1(pathname); {
 #if defined(LOGICAL_PATHNAMES)
  if (logpathnamep(pathname))
...
/* UP: Converts pathname into string.
 ...
 can trigger GC */
local maygc object whole_namestring (object pathname) { GCTRIGGER1(pathname); {
  {var uintC stringcount = 0;
...
/* UP: Returns the string for the directory of a pathname.
 ...
 < result: Normal-Simple-String
 can trigger GC */
local maygc object directory_namestring (object pathname) { GCTRIGGER1(pathname); {
...
/* UP: Returns the string identifying a file in its directory.
 ...
 < result: normal-simple-string
 can trigger GC */
local maygc inline object file_namestring (object pathname) { GCTRIGGER1(pathname); {
...
/* recalc_defaults_pathname();
 ...
 can trigger GC */
local maygc object recalc_defaults_pathname (void) { GCTRIGGER(); {
 #ifdef PATHNAME_WIN32
...
/* UP: Returns the default-pathname.
 ...
 < result: value of *DEFAULT-PATHNAME-DEFAULTS*, a pathname
 can trigger GC */
local maygc object defaults_pathname (void) { GCTRIGGER(); {
...
local maygc object merge_dirs (object p_directory, object d_directory, bool p_log,
                               bool wildp, bool called_from_make_pathname) { GCTRIGGER2(p_directory,d_directory); {
...
/* UP: copies a pathname.
 ...
 can trigger GC */
local maygc object copy_pathname (object pathname) { GCTRIGGER1(pathname); {
  pushSTACK(pathname);
...
local maygc void wildcard_diff_ab (object pattern, object sample,
                                   ...
                                   gcv_object_t* solutions) { GCTRIGGER2(pattern,sample); {
  {var chart cc;
  while (1) {
...
local maygc void version_diff   (object pattern, object sample, bool logical,
                                 ...
                                 gcv_object_t* solutions);
local maygc void host_diff (object pattern, object sample, bool logical,
                            const gcv_object_t* previous, gcv_object_t* solutions) { GCTRIGGER2(pattern,sample); {
...
local maygc void directory_diff_ab (object m_list, object b_list, bool logical,
                                    const gcv_object_t* previous,
                                    gcv_object_t* solutions) { GCTRIGGER2(m_list,b_list); {
...
local maygc void version_diff (object pattern, object sample, bool logical,
                               const gcv_object_t* previous, gcv_object_t* solutions)
{ GCTRIGGER2(pattern,sample); { /* logical is ignored */
...
/* On insertion of pieces of normal pathnames in logical pathnames:
 ...
 < result: converted Normal-Simple-String or the same Symbol/Number
 can trigger GC */
local maygc object logical_case (object string) { GCTRIGGER1(string); {
  if (!simple_string_p(string))
    return string;
  return string_upcase(string);
}}
/* The same, recursive like with SUBST: */
local maygc object subst_logical_case (object obj) { GCTRIGGER1(obj); {
...
/* On insertion of pieces of logical pathnames in normal pathnames:
 ...
 < result: converted Normal-Simple-String or the same Symbol/Number
 can trigger GC */
local maygc object customary_case (object string) { GCTRIGGER1(string); {
  if (!simple_string_p(string))
    return string;
 #if defined(PATHNAME_UNIX) || defined(PATHNAME_WIN32)
...
/* The same, recursive like with SUBST: */
local maygc object subst_customary_case (object obj) { GCTRIGGER1(obj); {
...
#endif
local maygc object translate_host (gcv_object_t* subst, object pattern,
                                   bool logical) { GCTRIGGER1(pattern); {
...
#undef RET_POP
#undef DEBUG_TRAN
local maygc object translate_pathname (gcv_object_t* subst, object pattern) { GCTRIGGER1(pattern); {
  {var bool logical = false;
  {var object item;
...
/* Converts an object into an absolute physical pathname and returns its
 ...
 can trigger GC */
global maygc object physical_namestring (object thing) { GCTRIGGER1(thing); {
  {var object pathname = coerce_pathname(thing);
  pathname = use_default_dir(pathname); /* insert default-directory */
...
/* UP: returns a namestring of a pathname for the operating system.
 ...
 can trigger GC */
local maygc object OSnamestring (object dir_namestring) { GCTRIGGER1(dir_namestring); {
...
/* UP: returns the current directory on the given drive.
 ...
 can trigger GC */
local maygc object default_directory_of (uintB drive, object pathname) { GCTRIGGER1(pathname); {
...
/* UP: Fills default-drive and default-directory into a pathname.
 ...
 can trigger GC */
local maygc object use_default_dir (object pathname) { GCTRIGGER1(pathname); {
...
local maygc void assure_dir_exists (struct file_status *fs,
                                    bool links_resolved, bool tolerantp) { GCTRIGGER(); {
...
/* UP: Return the current Directory.
 < result: current Directory (as Pathname)
 can trigger GC */
local maygc object default_directory (void) { GCTRIGGER(); {
...
/* UP: Turns a directory-namestring into one, that is suitably for DOS.
 ...
 can trigger GC */
local maygc object OSdirnamestring (object namestring) { GCTRIGGER1(namestring); {
...
/* UP: Changes the default-drive and its default-directory.
 ...
 can trigger GC */
local maygc void change_default (void) { GCTRIGGER(); {
...
/* tests, if a directory exists.
 ...
 can trigger GC */
local maygc bool directory_exists (object pathname) { GCTRIGGER1(pathname); {
...
/* Converts a directory pathname to an OS directory specification.
 ...
 can trigger GC */
global maygc object pathname_to_OSdir (object pathname, bool use_default) { GCTRIGGER1(pathname); {
...
/* Converts an OS directory specification to a directory pathname.
 ...
 can trigger GC */
global maygc object OSdir_to_pathname (const char* path) { GCTRIGGER(); {
...
#if (defined(WIN32_NATIVE)) && (!(defined(FILE_EXISTS_TRIVIAL)))
}
#endif
#if (defined(WIN32_NATIVE)) && (defined(FILE_EXISTS_TRIVIAL))
}
...
#endif
#if (defined(UNIX)) && (!(defined(FILE_EXISTS_TRIVIAL)))
}
#endif
#if (defined(UNIX)) && (defined(FILE_EXISTS_TRIVIAL))
}
...
local inline maygc void create_backup_file (char* pathstring,
                                            bool delete_backup_file) { GCTRIGGER(); {
...
(object namestring, bool delete_backup_file) { GCTRIGGER1(namestring); {
  with_sstring_0(namestring,O(pathname_encoding),namestring_asciz,
...
local maygc void check_file_re_open (object truename, direction_t direction) { GCTRIGGER1(truename); {
  {var uintB flags;
  switch (direction) {
...
local maygc object open_file (object filename, direction_t direction,
                              ...
                              if_does_not_exist_t if_not_exists) { GCTRIGGER1(filename); {
...
/* UP: extends a pathname by the file-information.
 ...
 can trigger GC */
local maygc void pack_full_info (decoded_time_t *timepoint, off_t *entry_size) { GCTRIGGER(); {
  {var object newlist;
...
local maygc void get_time_size (gcv_object_t *pathname,
                                decoded_time_t *timepoint, off_t *entry_size) { GCTRIGGER(); {
...
local maygc void with_stat_info_computed (struct file_status *fs) { GCTRIGGER(); {
...
/* UP: Extends the directory of a pathname by one component.
 ...
 can trigger GC */
local maygc object pathname_add_subdir (object pathname, object subdir) { GCTRIGGER2(pathname,subdir); {
...
/* push object in front of a list
 can trigger GC */
local inline maygc void push (gcv_object_t *head, gcv_object_t *tail) { GCTRIGGER(); {
  {var object new_cons = allocate_cons();
  Car(new_cons) = *head;
  Cdr(new_cons) = *tail;
  *tail = new_cons;
...
{ GCTRIGGER1(subdir); { /* copy pathname(STACK_0) and lengthen its directory by subdir: */
...
local maygc void check_sub_directory (object subdir, char* namestring_asciz) { GCTRIGGER1(subdir); {
#if defined(UNIX)
  struct stat status;
  int ret;
...
local maygc void directory_search_1subdir (object subdir, object namestring) { GCTRIGGER2(subdir,namestring); {
  with_sstring_0(namestring,O(pathname_encoding),namestring_asciz, {
...
local maygc object directory_search_hashcode (void) { GCTRIGGER(); {
...
local maygc bool directory_search_direntry_ok (object namestring,
                                               struct stat * statbuf) { GCTRIGGER1(namestring); {
  {var bool exists = true;
  with_sstring_0(namestring,O(pathname_encoding),namestring_asciz, {
    begin_system_call();
...
local maygc void directory_search_scandir (bool recursively, signean next_task,
                                           dir_search_param_t *dsp) { GCTRIGGER(); {
 #ifdef UNIX
  {
...
local maygc object directory_search (object pathname, dir_search_param_t *dsp) { GCTRIGGER1(pathname); {
  pathname = use_default_dir(pathname); /* insert default-directory */
...
/* UP: make sure that the supposed directory namestring ends with a slash
 ...
 can trigger GC */
local maygc object ensure_last_slash (object dir_string) { GCTRIGGER1(dir_string); {
...
/* decrements STACK by 1.
 can trigger GC */
local maygc object shorter_directory (object pathname, bool resolve_links) { GCTRIGGER1(pathname); {
  pathname = merge_defaults(coerce_pathname(pathname)); /* --> pathname */
  check_no_wildcards(pathname); /* with wildcards -> error */
  pathname = use_default_dir(pathname); /* insert default-directory */
  check_notdir(pathname); /* ensure that Name=NIL and Type=NIL */
...
/* UP: Initializes the pathname-system.
 init_pathnames();
 can trigger GC */
global maygc void init_pathnames (void) { GCTRIGGER(); {
 #ifdef PATHNAME_WIN32
...
(gcv_object_t *streamarg, bool child_inputp, Handle stdhandle,
 Handle * h, Handle * ph, Handle * hnull, bool * wait_p) { GCTRIGGER(); {
...
(gcv_object_t *ret, bool parent_inputp, Handle hparent_pipe, int childpid,
 gcv_object_t *enc, gcv_object_t *eltype, gcv_object_t *buffered) { GCTRIGGER(); {
...
local maygc sintL interpret_launch_priority (object priority_arg) { GCTRIGGER1(priority_arg); {
