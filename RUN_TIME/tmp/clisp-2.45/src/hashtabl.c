...
/* UP: Calculates the hashcode of an object with reference to a hashtable.
 ...
 < result: index into the index-vector
 can trigger GC */
local maygc uintL hashcode_user (object ht, object obj) { GCTRIGGER2(ht,obj); {
  {var uintL size = TheHashtable(ht)->ht_size;
...
/* Warn if a hash table is rehashed because of a GC, degrading performance.
 can trigger GC */
local maygc void warn_forced_gc_rehash (object ht) { GCTRIGGER1(ht); {
...
global maygc bool hash_lookup_user (object ht, object obj, bool allowgc,
                                    gcv_object_t** KVptr_, gcv_object_t** Iptr_) { GCTRIGGER2(ht,obj); {
  ASSERT(allowgc);
  pushSTACK(ht); pushSTACK(obj);
  if (!ht_validp(TheHashtable(ht))) /* hash-table must be reorganized */
    ht = rehash(ht);
...
/* Warn if adding an key to a hash table degrades its performance.
 can trigger GC */
local maygc void warn_key_forces_gc_rehash (object ht, object key) { GCTRIGGER2(ht,key); {
...
/* UP: Allocates the key-value-table for a new hash-table.
 ...
 can trigger GC */
local inline maygc object allocate_kvt (object weak, uintL maxcount) { GCTRIGGER1(weak); {
  if (nullp(weak)) {
...
local maygc uintL prepare_resize (object maxcount, object mincount_threshold,
                                  object weak) { GCTRIGGER3(maxcount,mincount_threshold,weak); {
...
/* UP: Enlarges or diminishes a hash-table
 ...
 can trigger GC */
local maygc object resize (object ht, object maxcount) { GCTRIGGER2(ht,maxcount); {
  pushSTACK(ht);
  {var uintL maxcountL =
...
/* check the :WEAK argument and return it
 can trigger GC */
local maygc object check_weak (object weak) { GCTRIGGER1(weak); {
 check_weak_restart:
...
/* error, if an argument is not a hash-table
 ...
 < hashtable
 can trigger GC */
local maygc object check_hashtable (object obj) { GCTRIGGER1(obj); {
...
/* return the hash table symbol
 ...
 can trigger GC - for user-defined ht_test */
global maygc object hash_table_test (object ht) { GCTRIGGER1(ht); {
