...
/* UP: expand all DEFTYPE definitions in the type spec
 ...
 can trigger GC */
global maygc object expand_deftype (object type_spec, bool once_p) { GCTRIGGER1(type_spec); {
  {var uintV max_depth =
...
/* UP: coerce STACK_1 to result_type (a sequence).
 ...
 can trigger GC */
local maygc Values coerce_sequence_check (object type, object result_type) { GCTRIGGER2(type,result_type); {
  pushSTACK(type);
  /* make new sequence: */
...
/* Creates a statistics record for the objects of a given type.
 can trigger GC */
local maygc object heap_statistics_record (object type, sintL n_instances, sintM n_bytes) { GCTRIGGER1(type); {
  pushSTACK(type);
